enyo.kind({
    name: "marinaeariel.PulloutDrawer",
    kind: enyo.Popup,
    floating: true,
    centered: false,
    modal: false,
    edge: "bottom",
    classes: "pullout-drawer enyo-unselectable",
    published: {
        autoCollapse: true,
        pulltabVisibility: false,
        pulltabPosition: -1 // -1 = left/top, 0 = center (pretty close), 1 = right/bottom
    },
    create: function () {
        this.inherited(arguments);
        this.pulltabPositionChanged();
        this.pulltabVisibilityChanged();
    },
    ...
    pulltabVisibilityChanged: function () {
        if (this.$.pulltab) {
            ...
            this.$.pulltab.removeClass(this.pulltabVisibility ? "pullout-drawer-invisible" : "pullout-drawer-visible");
        }
    },
    pulltabPositionChanged: function () {
        var positions = ["left", "top", "center", "right", "bottom"],
            position;
    ...
    createClient: function () {
        ...
        this.$.slider.createComponents(components, {
            owner: this
        });
    },
    hide: function (force) {
